...
export async function GET(
  ...
  if (chain !== "sol") {
    return NextResponse.json({ error: "Trades endpoint only supports Solana" }, { status: 400 });
  }

  try {
    const res = await fetch(
      `${BIRDEYE_BASE}/defi/txs/token?address=${address}&tx_type=all&limit=${limit}&offset=${offset}`,
